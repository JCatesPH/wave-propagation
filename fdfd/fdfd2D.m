...
thetat = asin(1 / sqrt(params.epsr) * sin(thetai));
ky2 = k0 * sqrt(params.epsr) * sin(thetat);
...
% Save the PML information.
sxfile = strcat(dir, "sx.csv");
dlmwrite(sxfile, sx);
...
if params.Floquet == 2
    bc = [exp(1i * ki_y * Wy)*ones(p,1); exp(1i * ky2 * Wy)*ones(q,1)];
    Dye = spdiags(bc, -params.Nx*(params.Ny-1), dye);
elseif params.Floquet == 1
